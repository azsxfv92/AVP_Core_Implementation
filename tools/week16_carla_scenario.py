...
class CarlaImagePublisher(Node):
    def __init__(self):
        ...
        super().__init__("week16_carla_scenario")
        ...
        self.output_dir = Path("results/week16/scenario")
        ...
        self.pub_speed = self.create_publisher(Float32, "/avp/vehicle/speed", 10)
        self.pub_autopilot = self.create_publisher(Bool, "/avp/vehicle/autopilot_active", 10)
        self.sub_accel = self.create_subscription(
            Float32, "/avp/vehicle/target_accel", self.on_target_accle, 10)
    ...
    def setup_carla(self):
        ...
        if self.town:
            self.get_logger().info(f"loading map: {self.town} (all of existed actors are deleted)")
        ...
        # generate obstacle
        obstacle_bp = self.blueprint_library.find("vehicle.audi.tt")
        hero_tf = self.vehicle.get_transform()
        fwd = hero_tf.get_forward_vector()
        obstacle_loc = carla.Location(
            x = hero_tf.location.x + fwd.x * self.obstacle_distance,
            y = hero_tf.location.y + fwd.y * self.obstacle_distance,
            z = hero_tf.location.z + 0.3,            
        )

        self.obstacle = self.world.try_spawn_actor(
            obstacle_bp, carla.Transform(obstacle_loc, hero_tf.rotation))
        
        if self.obstacle is None:
            self.get_logger().warn("Fail to spawn obstacle")
        else:
            self.obstacle.set_autopilot(False)
            self.get_logger().info(f"Complete to spawn obstacle ({self.obstacle_distance}m)")
    ...
    def on_control_tick(self):
        ...
        ap = Bool()
        ap.data = bool(self.autopilot_active)
        self.pub_autopilot.publish(ap)

        # stop when the parent command times out
        if self.last_accel_time is None:
            accel = 0.0
        else:
            dt = (self.get_clock().now() - self.last_accel_time).nanoseconds * 1e-9
            accel = -5.0 if dt > 0.5 else self.target_accel

        hazard = accel < 0.0

        if not self.hero_autopilot:
            ctrl = carla.VehicleControl()
            if hazard:
                ctrl.throttle = 0.0
                ctrl.brake = min(1.0, -accel / 5.0)
            else:
                ctrl.throttle = min(1.0, accel / 3.0)
                ctrl.brake = 0.0
            ctrl.steer = 0.0
    ...
    def cleanup(self):
        ...
        if self.obstacle is not None:
            ...
            self.get_logger().info("obstacle destroyed")
            self.obstacle = None
